...
/**
This function is used to simulate a c-scan disk scheduling algorithm by 
circularly scanning the disk, serving requests at the needed cylinders as they 
are reached. The function takes as arguments the starting location of the disk 
...
*/
int c_scan(vector<int> request_queue, vector<int> arrival_times, int head)
{
    ...
    while(requests_remaining > 0)
    {
        ...
        /// If the top of the disk is reached, move the to the bottom.
        if(head >= DISK_SIZE-1)
            head = 0;

        /// Else, step the head up.
        else
            head++;
